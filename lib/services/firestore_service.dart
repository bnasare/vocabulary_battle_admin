...
class FirestoreService {
  ...
  /// Delete a game session (admin only - calls Cloud Function)
  Future<void> deleteGame({
    required String sessionId,
    String? reason,
  }) async {
    final functions = FirebaseFunctionsConfig.instance;
    await functions.httpsCallable('deleteGame').call({
      'sessionId': sessionId,
      'reason': reason,
    });
  }
  ...
  /// Get game history
  Future<List<GameSession>> getGameHistory(String userId,
      {int limit = 10}) async {
    final snapshot = await _firestore
        .collection(FirestoreCollections.gameHistory)
        .where(Filter.or(
  ...
  /// Get set of user IDs who are currently in active games
  Future<Set<String>> getUsersWithActiveGames() async {
    final snapshot = await _firestore
        .collection(FirestoreCollections.gameSessions)
        .where('status', whereIn: [
      GameStatus.preparation,
    ...
    for (final doc in snapshot.docs) {
      ...
      userIds.add(session.player1Id);
      userIds.add(session.player2Id);
    }
